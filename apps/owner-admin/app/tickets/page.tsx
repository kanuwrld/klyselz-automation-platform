...
import { getTicketsForAgency, getTicketsForClient } from "@/lib/data";
...
export default async function TicketsPage() {
  ...
  const tickets = session.role === "agency"
    ? await getTicketsForAgency()
    : session.clientId
      ? await getTicketsForClient(session.clientId)
      : [];

  const canCreate = session.role === "client" || session.role === "agency";
    ...
    <main className="mx-auto max-w-6xl px-4 py-8">
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {session.role === "agency"
              ? "Alle Kundenanfragen: antworten, Status ändern, schließen."
              : "Schreib KLYSELZ direkt. Antwort bleibt im Dashboard sichtbar."}
          </p>
        </div>
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold">
          {session.role === "agency" ? "Alle Tickets" : "Meine Tickets"}
        </h2>
        <Card>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {session.role === "agency" && <TableHead>Kunde</TableHead>}
            ...
            <TableBody>
              {tickets.length === 0 && (
                <TableRow>
                  <TableCell colSpan={session.role === "agency" ? 6 : 5} className="text-center text-sm text-muted-foreground">
                    Noch leer. {session.role === "client" ? "Erstelle dein erstes Ticket oben." : "Warten auf Kundenanfragen."}
                  </TableCell>
                </TableRow>
              )}
              {tickets.map((t) => (
